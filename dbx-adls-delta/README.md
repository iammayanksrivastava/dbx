...
```
dbx execute --cluster-name=<name of interactive cluster> --job=dbx-adls-delta-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=dbx-adls-delta-sample-integration-test --files-only
dbx launch --job=dbx-adls-delta-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `dbx_adls_delta/__init__.py` file and tag the current code version:
```
